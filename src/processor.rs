...
pub trait ConfigurableFilter {
    ...
    fn requires_fields(
        ...
        match optional_config {
            &Some(ref config) => for required in required_fields {
...
pub trait InputProcessor: ConfigurableFilter {
    ...
    fn invoke(
        ...
        handle_func: fn(tx: SyncSender<String>, config: Option<HashMap<String, String>>),
    ) -> Receiver<String> {
        let (tx, rx) = sync_channel(10000);
        let conf = config.clone();

        let run_loop = thread::Builder::new()
...
pub trait OutputProcessor: ConfigurableFilter {
    ...
    fn invoke(
        ...
        rx: Receiver<String>,
        config: &Option<HashMap<String, String>>,
        handle_func: fn(rx: Receiver<String>, config: Option<HashMap<String, String>>),
    ) -> Result<JoinHandle<()>, String> {
        let conf = config.clone();
