    ...
    match configuration {
        Ok(conf) => {
            ...
            let data_output = match output.0.as_ref() {
                "stdout" => outputs::stdout::Stdout::new(dataoutput_name.to_owned())
                    .start(data_input, oargs),
                "network" => outputs::network::Network::new(dataoutput_name.to_owned())
                    .start(data_input, oargs),
                "file" => outputs::file::RotatingFile::new(dataoutput_name.to_owned())
                    .start(data_input, oargs),
                "elasticsearch" => outputs::elasticsearch::Elasticsearch::new(
                    dataoutput_name.to_owned(),
                ).start(data_input, oargs),
