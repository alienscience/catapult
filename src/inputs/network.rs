...
impl InputProcessor for Network {
    ...
    fn handle_func(tx: SyncSender<String>, oconfig: Option<HashMap<String, String>>) {
        ...
        loop {
            let mut buf = [0; 1024];
            if let Ok((bytes, _)) = udp.recv_from(&mut buf) {
                let l = String::from_utf8_lossy(&buf[0..bytes]).into_owned();
                match tx.try_send(l) {
                    Ok(()) => {}
                    Err(e) => println!("Unable to send line to processor: {}", e),
                }
            }
        }
    }
