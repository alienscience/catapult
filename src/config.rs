...
use nom::{alphanumeric, multispace, not_line_ending, space, IResult};
use nom::IResult::*;
...
named!(quoted_string(&[u8]) -> &str,
  do_parse!(
    tag!("\"") >>
    qs: map_res!(take_until!("\""), str::from_utf8) >>
    tag!("\"") >>
    (qs)
  )
);

named!(object_symbol_name(&[u8]) -> &str,
    ...
    match f.read_to_string(&mut s) {
        Ok(_) => {
            ...
            match configuration(&source) {
                ...
                Error(e) => Err(format!("Parse error: {:?}", e)),
                Incomplete(e) => Err(format!("Incomplete content -> await: {:?}", e)),
            }
        }
